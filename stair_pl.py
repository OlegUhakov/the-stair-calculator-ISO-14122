...
st.set_page_config(page_title="Kalkulator schodów ISO 14122-3", layout="wide")

st.title("Kalkulator schodów przemysłowych")
st.caption("Obliczenia i weryfikacja zgodnie z ISO 14122-3 (Bezpieczeństwo maszyn)")
...
with col_sidebar:
    ...
    H = st.number_input(
        "Całkowita wysokość podniesienia (H), mm", min_value=300, max_value=4000, value=1500, step=5,
    )

    Pup = st.number_input("Pup (przesunięcie pionowe u góry), mm", min_value=0, max_value=1000, value=0, step=5)
    Pdown = st.number_input("Pdown (przesunięcie pionowe u dołu), mm", min_value=0, max_value=1000, value=0, step=5)
    B = st.number_input("Platforma dolna (B), mm", min_value=0, max_value=1000, value=0, step=5)
    r = st.number_input("Nachodzenie (r), mm", min_value=0, max_value=50, value=10, step=1)

    st.markdown("---")

    N = st.slider("Stopnie (N)", min_value=1, max_value=30, value=8)
    g = st.slider("Stopnica (g), mm", min_value=150, max_value=320, step=1, value=280)
    ...
    st.markdown("---")
    st.number_input("Podstopień (h), mm", value=float(round(h_actual, 1)), disabled=True, format="%.1f", key="__dh")
    st.number_input("Stopnica (g), mm", value=float(g_actual), disabled=True, format="%.1f", key="__dg")
    st.number_input("Całkowita szerokość stopnicy (t), mm", value=float(t_total), disabled=True, format="%.1f", key="__dt")
    st.number_input("L (rozpiętość pozioma), mm", value=float(round(L, 0)), disabled=True, format="%.0f", key="__dl")

    st.markdown("---")
    st.subheader("Odległość między stopniami")
    step_diagonal = math.sqrt(g_actual ** 2 + h_actual ** 2)
    st.number_input("Wzdłuż nachylenia, mm", value=float(round(step_diagonal, 1)), disabled=True, format="%.1f", key="__ddiag")

    st.markdown("---")
    st.subheader("Przesunięcia stopni")
    ...
    st.number_input("Ad (wzdół schodów, dół), mm", value=float(round(Ad, 1)), disabled=True, format="%.1f")
    st.number_input("Aup (wzdół schodów, góra), mm", value=float(round(Aup, 1)), disabled=True, format="%.1f")

    if angle < 45:
        stair_type = "Schody (20°–45°)"
        angle_min_ok, angle_max_ok = 20, 45
        g_min_ok = 200
        h_max_ok = 240
        blondel_applies = True
    else:
        stair_type = "Drabiny przystawne (45°–75°)"
...
with col_main:
    ...
    with col_metrics:
        ...
        if B > 0:
            st.metric("Platforma (B)", f"{B} mm")
        if Pdown > 0:
            st.metric("Pdown", f"{Pdown} mm")
        st.metric("Stopnie (N)", f"{N}")
        st.metric("Podstopień (h)", f"{h_actual:.1f} mm")
        st.metric("Stopnica (g)", f"{g_actual:.1f} mm")
        st.metric("L (rozpiętość pozioma)", f"{L:.0f} mm")
        st.metric("Kąt (α)", f"{angle:.1f}°")
        st.metric("Wzdłuż nachylenia", f"{step_diagonal:.1f} mm")
        st.metric("Całkowita szerokość stopnicy (t)", f"{t_total:.1f} mm")
        st.metric("Nachodzenie (r)", f"{r} mm")
    ...
    with col_draw:
        ...
        st.subheader("Widok z boku")
        st.components.v1.html(svg_wrapper, height=svg_h + 20)

    if blondel_applies:
        st.info(
            f"**Wzór Blondela:** obliczona wartość to **{blondel:.1f} mm** "
            f"(zakres ISO: 600–660 mm)."
        )

    st.markdown("---")
    st.subheader("Zgodność z normami")

    st.info(f"Określony typ: **{stair_type}**")

    if is_all_valid:
        st.success("W pełni spełnia wymagania!")
    else:
        st.error("Wykryto naruszenia:")
        if not is_valid_angle:
            st.warning(
                f"Kąt nachylenia {angle:.1f}° poza dozwolonym zakresem ({angle_min_ok}°–{angle_max_ok}°)."
            )
        if not is_valid_blondel:
            st.warning(
                f"Wzór Blondela {blondel:.0f} mm poza dozwolonym zakresem (600–660 mm)."
            )
        if not is_valid_g:
            st.warning(f"Głębokość stopnicy {g_actual:.1f} mm zbyt mała (min. {g_min_ok} mm).")
        if not is_valid_h:
            st.warning(
                f"Wysokość podstopnia {h_actual:.1f} mm przekracza maksymalnie dozwoloną ({h_max_ok} mm)."
            )
